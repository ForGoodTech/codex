...
GitHub release tag, `rust-v0.149.1` by default, or an explicitly provided tag.

```shell
cd ext/docker/pi

# Build an image tagged "my-codex-docker-image" using rust-v0.149.1.
./build_image.sh
...
./build_image.sh codex-release rust-v0.149.1
```
...
environment. If no tag is provided, the script uses `rust-v0.149.1`.

What the script does:

1. Prepares Docker build artifacts under `codex-cli/dist/`.
2. Selects the release tag, `rust-v0.149.1` by default unless a tag is provided
   explicitly.
