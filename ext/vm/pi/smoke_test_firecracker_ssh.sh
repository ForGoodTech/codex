...
for cmd in curl ip jq rg ssh sudo; do
...
if [[ -z "$ARTIFACT_ROOT" ]]; then
  CODEX_RELEASE_TAG=${CODEX_RELEASE_TAG:-rust-v0.128.0}
...
PROXY_PORT=$(jq -r '.proxyPort // 9395' "$MANIFEST_PATH")
...
run_remote_check() {
  ssh -o BatchMode=yes \
    ...
    "set -euo pipefail; \
     ...
     echo 'sshd_processes='; \
     pgrep -af sshd || true; \
     echo 'codex_proxy_processes='; \
     pgrep -af codex-app-server-proxy || true; \
     echo 'listeners='; \
     ss -ltn '( sport = :22 or sport = :${PROXY_PORT} )' || true"
}
...
wait_for_ssh
REMOTE_OUTPUT=$(run_remote_check) || {
  TEST_FAILED=1
  echo "Remote SSH health check failed." >&2
  echo "Serial log:" >&2
  sed -n '1,240p' "$SERIAL_LOG" >&2 || true
  echo "Firecracker log:" >&2
  sed -n '1,240p' "$FC_LOG" >&2 || true
...
echo "Firecracker SSH smoke test passed."
echo "Guest IP: $GUEST_IP"
echo "Tap device: $TAP_DEV ($HOST_IP_CIDR)"
echo "$REMOTE_OUTPUT"
